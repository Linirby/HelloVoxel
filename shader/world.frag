...
float compute_shadow(vec4 light_clip, float ndotl) {
	...
	float bias = clamp(0.0003 * slope, 0.00005, 0.004);
...
void main() {
	...
	float shadow = 0.0;
	if (ndotl > 0.0) {
		float raw = compute_shadow(v_light_pos, ndotl);
		float terminator = smoothstep(0.0, 0.1, ndotl);
		shadow = raw * terminator;
	}
